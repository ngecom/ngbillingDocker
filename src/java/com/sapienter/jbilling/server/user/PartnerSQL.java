...
public interface PartnerSQL {
    ...
    static final String duePayout = 
            "select p.id " +
            "  from " + ServerConstants.TABLE_PARTNER + " p, " + 
                        ServerConstants.TABLE_BASE_USER + " bu " +
    ...
    static final String lastPayout =
            "select max(pp.id) " +
            "  from " + ServerConstants.TABLE_PARTNER_PAYOUT + " pp, " +
                        ServerConstants.TABLE_PAYMENT + " pa " +
            " where partner_id = ? " +
            "   and pp.payment_id = pa.id " +
            "   and pa.result_id in (" + ServerConstants.RESULT_OK + "," +
                                         ServerConstants.RESULT_ENTERED + ")";
            
    // this query is a real pity that is not doable with esql because of the
    // date restriction
    static final String paymentsInPayout = 
            "select pa.id " +
            "  from " + ServerConstants.TABLE_PAYMENT + " pa, " +
                        ServerConstants.TABLE_BASE_USER + " bu, " +
                        ServerConstants.TABLE_CUSTOMER + " cu " +
    ...
    static final String list = 
            ...
            "  from " + ServerConstants.TABLE_PARTNER + " pa, " + 
                        ServerConstants.TABLE_BASE_USER + " bu " +
            " where pa.user_id = bu.id " +
            "   and bu.deleted = 0 " +
            "   and bu.entity_id = ? " +
    ...
    static final String countCustomers =
            "select count(*) " +
            "  from " + ServerConstants.TABLE_CUSTOMER + " c, " +
                        ServerConstants.TABLE_BASE_USER + " bu " +
