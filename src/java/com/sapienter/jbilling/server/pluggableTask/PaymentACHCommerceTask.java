    ...
    public boolean process(PaymentDTOEx paymentInfo)
        ...
        try {
            ...
            if (paymentAuthDTO.getCode1().equals(PAYMENT_SUCCESS_CODE)) {
                paymentInfo.setPaymentResult(new PaymentResultDAS().find(CommonConstants.RESULT_OK));
            } else {
                paymentInfo.setPaymentResult(new PaymentResultDAS().find(CommonConstants.RESULT_FAIL));
            }
            return false;
        } catch (HttpException he) {
            logger.error("Error during payment processing", he);
            paymentInfo.setPaymentResult(new PaymentResultDAS().find(CommonConstants.RESULT_UNAVAILABLE));
            return true;
